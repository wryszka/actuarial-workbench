 ...
 *
 * Cards are organised by CATEGORY (a controlled vocabulary — see CATEGORIES)
 * and carry a KIND badge (what kind of thing it is / how finished it is).
 * The page groups them into one section per category, in the order below.
 * To add a project: give it a `category` from CATEGORIES and a `kind` from
 * KINDS — do NOT invent a free-text label, so the taxonomy stays clean.
 */
...
export type SmallProjectCategory =
  | 'pricing-reserving'
  | 'claims-underwriting'
  | 'finance'
  | 'platform-security'
...
export const CATEGORIES: { id: SmallProjectCategory; label: string }[] = [
  { id: 'pricing-reserving', label: 'Pricing & Reserving' },
  { id: 'claims-underwriting', label: 'Claims & Underwriting' },
  { id: 'finance', label: 'Finance & Reconciliation' },
  { id: 'platform-security', label: 'Platform & Security' },
  { id: 'enablement', label: 'Enablement & Workshops' },
  { id: 'partner', label: 'Partner' },
];

/** What kind of thing the card is — drives the coloured badge. */
export type SmallProjectKind =
  | 'live-app'
  | 'accelerator'
  | 'demo'
  | 'workshop'
...
export const KINDS: Record<SmallProjectKind, { label: string; badge: string }> = {
  'live-app':    { label: 'Live app',    badge: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  'accelerator': { label: 'Accelerator', badge: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
  'demo':        { label: 'Demo',        badge: 'bg-blue-50 text-blue-700 border-blue-200' },
  'workshop':    { label: 'Workshop',    badge: 'bg-amber-50 text-amber-700 border-amber-200' },
  'poc':         { label: 'POC',         badge: 'bg-gray-100 text-gray-600 border-gray-300' },
  'resource':    { label: 'Resource',    badge: 'bg-slate-100 text-slate-600 border-slate-300' },
};
...
export interface SmallProject {
  ...
  category: SmallProjectCategory;
  kind: SmallProjectKind;
  tag?: string;                // optional fine-grained sub-label (secondary)
...
export const SMALL_PROJECTS: SmallProject[] = [
  ...
  },
  {
    ...
    tag: 'Designer',
    links: [
      { label: 'Run doc', href: 'https://docs.google.com/document/d/1G8PnkyNgyQIKY6uVNZqS7TdlQEmwyi0UD8d5CZsxO2w/edit' },
      { label: 'Session plan', href: 'https://docs.google.com/document/d/13tUUsXneQs1rMKv_bUZHk64SrZ8G32mC3K_S7Sry_FA/edit' },
  ...
  {
    title: 'Databricks Foundations Workshop',
...
/** Projects grouped by category, in CATEGORIES order; empty categories dropped. */
export function projectsByCategory(): { id: SmallProjectCategory; label: string; projects: SmallProject[] }[] {
  return CATEGORIES
    .map(({ id, label }) => ({ id, label, projects: SMALL_PROJECTS.filter((p) => p.category === id) }))
